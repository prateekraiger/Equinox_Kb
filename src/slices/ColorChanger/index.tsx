          ...
          {KEYCAP_TEXTURES.map((texture) => (
            <li key={texture.id}>
              <button
                ...
                className={clsx(
                  "flex aspect-square flex-col items-center justify-center rounded-lg order-2 p-2 hover:scale-105 motion-safe transition-all motion-safe:duration-300",
                  selectedTextureId === texture.id
                    ? "border-[#81BFED] bg-[#818FED]/20"
                    : "cursor-pointer border-gray-300 hover:border-gray-500",
                  isAnimating && "cursor-not-allowed  opacity-50"
                )}
              >
                <div className="mb-3 overflow-hidden rounded border-2 border-black bg-grey-100">
                  <Image
                    src={texture.path}
                    alt={texture.name}
                    width={400}
